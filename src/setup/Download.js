...
class Download extends React.Component
{
  ...
  download =()=>{
    ...
    downloader.getAttributes()
      .then(res=>{this.setState({items:{...this.state.items,attributes:true}});return downloader.getAttributeSets()})
      .then(res=>{this.setState({items:{...this.state.items,attributeSets:true}});return downloader.getTaxRules()})
      .then(res=>{this.setState({items:{...this.state.items,tax:true}});return downloader.getCategories()})
      .then(res=>{this.setState({items:{...this.state.items,store:true}});return downloader.getStores()})
      .then(res=>{this.setState({items:{...this.state.items,categories:true}});return downloader.getProducts()})
      .then(res=>{this.setState({items:{...this.state.items,product:true}});return downloader.getProducts()})
